...
def non_extreme_distribution(partition_class_bcs_assignment, low=5, high=20):
    num_extreme = [x for x in partition_class_bcs_assignment.values() if not low < len(x['classes']) < high]
    class_sizes = [len(x['classes']) for x in partition_class_bcs_assignment.values()]
    print(f'Partition sizes: {class_sizes}')
    return len(num_extreme) / len(partition_class_bcs_assignment), np.array(class_sizes)
